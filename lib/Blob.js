...
export default class Blob extends VersionControlEntity {
    ...
    loadMetadata(){
        let that = this;
        if (!that._meta) {
            that._meta = that.readTransaction(function(){
                return that.store.loadRevisionInfo({
    ...
    loadContent(){
        let that = this;
        if (!that._content){
            that._content = that.readTransaction(function() {
    ...
    newBlob(options){
        ...
        return that.writeTransaction(function(){
            return that._serializeContent(options.content)
            .then(function(newContent){
                let newContentHash = Digest.digest(newContent);
                return that.store.loadRevisionInfoByHash({
                    hash : newContentHash
    ...
    _buildDiff(oldContent, newContent){
        if (!oldContent || !oldContent.length) 
            return null;
