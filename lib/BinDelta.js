// This source was copied from the 'node-git-core' project
// https://raw.githubusercontent.com/tarruda/node-git-core/master/src/js/delta.js
// -----------------------------------------------------------------------------
...
function chooseMatch(source, sourcePositions, target, targetPos) {
  ...
  for (i = 0;i < sourcePositions.length;i++) {
    ...
    if (rv && spos < (rv.offset + rv.length))
      ...
      continue;
    while (spos < source.length && tpos < target.length && source[spos++] === target[tpos]) {
      len++;
      tpos++;
    }
    if (!rv) {
