...
export default class SqliteStore {
    ...
    transaction(method, ...params) {
        ...
        if (!that._transaction) {
            that._transaction = Promise.resolve();
        }
        return that._transaction = that._transaction.then(function(){
            return that.run('BEGIN TRANSACTION').then(function(){
                return (method.bind(that))(...params);
            }).then(function(result){
                return that.run('COMMIT TRANSACTION').then(function(){
                    return result;
                });
            }, function(err){
                return that.run('ROLLBACK TRANSACTION').then(function(){
                    throw err;
                });
            })
        });
