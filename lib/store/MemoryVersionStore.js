...
export default class MemoryVersionStore extends MemoryStore {
    ...
    loadPaths(options){
        ...
        return Promise.resolve().then(function(){
            ...
            if (options.pids){
                let idsToPaths = that._idsToPaths = that._idsToPaths || {};
                options.pids.forEach(function(pid){
                    let path = idsToPaths[pid];
                    result[path] = pid;
                });
            } else {
                let paths = options.paths || [];
    ...
    storeVersionParents(options){
        ...
        return Promise.resolve().then(function(){
            ...
            let oldParents = parentsIndex[vid] || [];
            oldParents.forEach(function(parent){
                let children = childrenIndex[parent];
                if (children){
                    delete children[vid];
                }
            });
            parentsIndex[vid] = parents;
            parents.forEach(function(parent){
                let children = childrenIndex[parent] = childrenIndex[parent] || {};
                children[vid] = true;
            });
            return parents;
        });
    ...
    loadVersionChildren(options){
        ...
        return Promise.resolve().then(function(){
            ...
            let vid = options.vid;
            if (!vid) throw new Error('Version id is not defined');
            let children = childrenIndex[vid] || {};
