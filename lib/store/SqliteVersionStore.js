...
export default class SqliteVersionStore extends SqliteStore {
    ...
    storeRevisionInfo(options){
        ...
        return that.writeTransaction(function(){
            return that.run(
                `INSERT OR IGNORE INTO ${that.t.revs}(length, hash, contentId, diffId)
    ...
    storeVersionRevisions(options){
        ...
        return that.writeTransaction(function(){
            ...
            if (options.replace) {
                promise = that.run(
                    `DELETE FROM ${that.t.versionRevisions}
                    WHERE versionId=$versionId`, {
                    $versionId: versionId
                });
            } else {
            ...
            return promise.then(function(){
                return Promise.all(pathIdList.map(function(pathId){
                    ...
                    return that.run(
                        `INSERT OR REPLACE INTO ${that.t.versionRevisions}
    ...
    loadVersionRevisions(options){
        ...
        return that.readTransaction(function(){
            let versionId = options.versionId;
            return that.all(
                `SELECT * FROM ${that.t.versionRevisions} 
                WHERE versionId=$versionId`, {
                $versionId: versionId
            }).then(function(result){
