...
export default class SqliteVersionStore extends SqliteStore {
    ...
    get t(){
        if (!this._tableSchema) {
            ...
            this._tableSchema = {
                ...
                versionParents : prefix + 'version_parent',
            };
        }
    ...
    loadPaths(options){
        ...
        return Promise.resolve().then(function(){
            ...
            if (options.paths) {
                ...
                sql = `SELECT * FROM ${that.t.path} WHERE pid=$path`;
                paths = options.pids || [];
            }
            if (!paths.length)
                return [];
    ...
    storeVersionRevisions(options){
        ...
        return Promise.resolve().then(function(){
            ...
            return that.run(
                ...
                return that.loadVersionRevisions({vid});
            });
        });
    }
 ...
 // ------------------------------------------------------------------------
     ...
     * @param options.vid
     *            version identifier
     * @param options.parents a list of parent version identifiers
     */
    storeVersionParents(options){
        ...
        return Promise.resolve().then(function(){
            ...
            return that.run(
                `DELETE FROM ${that.t.versionParents} WHERE vid=$vid`, {
                $vid: vid
            }).then(function(){
                return Promise.all(parents.map(function(parent){
                    return that.run(
                            `INSERT INTO ${that.t.versionParents}(vid,parent)
                             VALUES($vid, $parent)`, {
                             $vid:vid,
                             $parent: parent
                        });
                ...
                return that.loadVersionParents({vid:vid});
            });
        });
    }
    ...
    loadVersionParents(options){
        ...
        return Promise.resolve().then(function(){
            ...
            if (!vid) throw new Error('Version id is not defined');
            return that.all(
                `SELECT * FROM ${that.t.versionParents} WHERE vid=$vid`, {
                $vid: vid
            }).then(function(info){
    ...
    loadVersionChildren(options){
        ...
        return Promise.resolve().then(function(){
            ...
            if (!vid) throw new Error('Version id is not defined');
            return that.all(
                `SELECT * FROM ${that.t.versionParents} WHERE parent=$vid`, {
                $vid: vid
            }).then(function(info){
                let list = info.result;
                let result = [];
