...
export default class SqliteVersionStore extends SqliteStore {
    ...
    loadBlob(options) {
        ...
        return Promise.resolve().then(function(){
            return that.get(
                `SELECT blobId, length, hash, content FROM ${that.t.blob} WHERE blobId = $blobId`,
    ...
    storeVersionRevisions(options){
        ...
        return Promise.resolve().then(function(){
            ...
            return that.run(
                `DELETE FROM ${that.t.changeset} WHERE versionId=$versionId ${where}`, {
                $versionId: versionId
            }).then(function(){
                return Promise.all(pathIdList.map(function(pathId){
                    let revisionId = mapping[pathId];
                    return that.run(
                        `INSERT INTO ${that.t.changeset}(versionId, pathId, revisionId)
    ...
    loadVersionRevisions(options){
        ...
        return Promise.resolve().then(function(){
            let versionId = options.versionId;
            return that.all(
                `SELECT * FROM ${that.t.changeset} WHERE versionId=$versionId`, {
                $versionId: versionId
            }).then(function(result){
